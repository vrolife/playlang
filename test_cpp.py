...
def test(cls, source):
    ...
    if export_dir is not None:
        ...
        shutil.copy('scanner.hpp', folder)
        shutil.copy('playlang.hpp', folder)

        with open(os.path.join(folder, 'parser.hpp'), 'w') as f:
            CPlusPlus.generate(compiler, f, cls.__name__.lower())
            
        
        with open(os.path.join(folder, 'tests.cpp'), 'w') as f:
            f.write(source)
...
class TestCPlusPlus(unittest.TestCase):
    def test_calc(self):
        status = test(ParserCalc, """
#define PLAYLANG_PARSER_HEADER parser.hpp
